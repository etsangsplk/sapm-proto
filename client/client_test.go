...
func TestDefaults(t *testing.T) {
	...
	assert.Equal(t, defaultNumWorkers, uint64(len(c.workers)))
	w := <-c.workers
...
func TestBadRequest(t *testing.T) {
	...
	// 1. tell workers to retry failing requests up to 4 times
	c, err := New(
		defaultEndpointOption,
		WithHTTPClient(newMockHTTPClient(transport)),
		WithMaxRetries(4),
	...
	for _, code := range []int{400, 401} {
		...
		// 2. ensure export resulted in an error
		require.NotNil(t, err)
		assert.Equal(t, err.Error(), "dropping request: server responded with: " + strconv.Itoa(code))

		requests := transport.requests()

		// 3. ensure transport only received one HTTP request i.e, no retry attempts were made
		require.Len(t, requests, 1)
		assertRequestEqualBatch(t, requests[0].r, batch)
