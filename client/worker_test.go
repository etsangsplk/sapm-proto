...
func TestWorkerSend(t *testing.T) {
	...
	w := newWorker(newMockHTTPClient(transport), "", "", 0)
...
func TestWorkerSendErrors(t *testing.T) {
	...
	assert.Equal(t, 400, sendErr.httpCode)
	...
	sendErr = w.send(ctx, sr)
	require.NotNil(t, sendErr)
	assert.Equal(t, 500, sendErr.httpCode)
	...
	sendErr = w.send(ctx, sr)
	require.NotNil(t, sendErr)
	assert.Equal(t, 429, sendErr.httpCode)
	assert.False(t, sendErr.permanent)
	assert.Equal(t, sendErr.retryDelaySeconds, default429BackoffSeconds)
	...
	sendErr = w.send(ctx, sr)
	require.NotNil(t, sendErr)
	assert.Equal(t, sendErr.Error(), "Post : test error")
	assert.Equal(t, 0, sendErr.httpCode)
	assert.False(t, sendErr.permanent)
	assert.Equal(t, sendErr.retryDelaySeconds, 0)
}
